...
class SpotDataManagerFather():
    print("spot 运行时间 : 【{}】".format(datetime.datetime.now()))  # 跟中性实盘对比，看下什么原因导致的选币过快
...
class DataManagerSon():
    ...
    def __init__(self, time_interval, exchange,sql,db_name):
        ...
        self.name = 'b_spot_' + time_interval  # 子类名称，用于区分
        self.sql = sql # 数据库
    ...
    def scheduler(self):  # 供定时器调用
        ...
        exchange_info = robust(self.exchange.publicGetExchangeInfo, )
        ...
        if self.re_download_all_his_coin_data:
            ...
            ic('spot 数据实时更新完毕', (datetime.datetime.now() - run_time).seconds, symbol_candle_data['BTCUSDT'].head(2))
            ic('spot 数据实时更新完毕',(datetime.datetime.now()- run_time).seconds,symbol_candle_data['BTCUSDT'].tail(2))
            ...
            for symbol in symbol_list:# 这个代码好像有点问题，除非在获取一次symbol_list不然好像没有意义,但是只有刚刚上架的币
                ...
                if symbol not in data_symbol_list:
                    ...
                    print("spot 补全数据 {}".format(symbol))
                    _, _df = get_data(symbol, self.exchange, MAX_KEEP_LEN, self.time_interval, run_time,'spot')
                    self.sql.create_talbe(_df, self.db_name , self.name, if_exists="append",text = "spot 补全数据 {}".format(symbol))
                    # 这里存入新的数据，但是不更新数字币列表了，在后面在进行更新


            # 在第一次运行完会再次执行下面的内容，直到第二次才是单独获取
            # =====并行获取所有币种的(1小时K线),增量更新(10)条数据
            symbol_candle_data = get_binance_history_candle_data(self.exchange, symbol_list, self.time_interval, run_time, 10,'spot')
            ...
            try:
                self.sql.create_talbe(df, self.db_name, self.name, if_exists="append",text = 'spot 存入新增数据')
            except:
                pass
            ...
            if len(symbol_df) != 0 :
                self.sql.create_talbe(symbol_df, self.db_name , self.name + "_symbol", if_exists="replace", text = "spot 更新 symbol_list 表")
        ...
        print(self.name + " spot 数据获取完毕 {}m {:.2f}s at {}\n".format(minutes, seconds, current_time))
